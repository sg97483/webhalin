...
def handle_ticket(driver, park_id, ticket_name):
    ...
    # ✅ 19160 (하이파킹 문정프라비다점) 전용 할인 처리
    if park_id == 19160:
        ticket_map = {
            "평일 3시간권": "평일3시간권",
            "평일 12시간권(월)": "평일12시간권",
            "평일 12시간권(화~금)": "평일12시간권",
            "휴일 당일권": "주말일일권",
        }

        if ticket_name not in ticket_map:
            print(f"ERROR: 19160에서 지원하지 않는 ticket_name: {ticket_name}")
            return False
        ...
        try:
            ...
            for option in select.options:
                if option.text.strip().startswith(target_text):
                    select.select_by_visible_text(option.text.strip())
                    print(f"DEBUG: '{option.text}' 옵션 정확 매칭 및 선택 완료.")
                    matched = True
                    break

            if not matched:
                print(f"ERROR: '{target_text}'로 시작하는 옵션을 찾을 수 없습니다.")
                return False
            ...
            print(f"ERROR: 19160 처리 중 예외 발생: {e}")
            return False
    ...
    if park_id == 19561:
        ...
        if ticket_name not in ticket_map:
            print(f"ERROR: 19561에서 지원하지 않는 ticket_name: {ticket_name}")
            return False
        ...
        try:
            ...
            print(f"ERROR: 19561 처리 중 예외 발생: {e}")
            return False
        ...
        if park_id in mapIdToWebInfo:
            ...
            for index in range(1, count + 1):
                ...
                if ori_car_num[-7:] == td_car_num or ori_car_num == td_car_num:
                    ...
                    if park_id in [19004, 19600, 19860, 19160, 29184, 29136,19148]:
